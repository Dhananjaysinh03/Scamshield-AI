...
> = {
  low: {
    title: "Looks safer",
    advice: "We didn’t find strong scam signs. Still avoid sharing OTPs or passwords.",
    tone: "text-accent",
    chip: "bg-accent-soft text-accent",
  },
  medium: {
    title: "Be careful",
    advice: "Some warning signs showed up. Don’t click links or send money until you verify another way.",
    tone: "text-amber-700 dark:text-amber-400",
    chip: "bg-amber-500/15 text-amber-800 dark:text-amber-400",
  },
  high: {
    title: "Likely a scam",
    advice: "This message shows common scam patterns. Do not click links, share OTPs, or pay anything.",
    tone: "text-orange-700 dark:text-orange-400",
    chip: "bg-orange-500/15 text-orange-800 dark:text-orange-400",
  },
  critical: {
    title: "Dangerous scam",
    advice: "Strong signs of fraud. Block the sender, don’t reply, and never share bank or OTP details.",
    tone: "text-danger",
...
export function ScanResults({ result }: { result: ScanResult | null }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-panel p-5 shadow-sm sm:p-6 dark:shadow-none">
      ...
      <p className={`mt-3 font-display text-2xl font-bold tracking-tight sm:text-3xl ${copy.tone}`}>
        {copy.title}
      </p>
      ...
      {result.urls.length > 0 ? (
        <div className="mt-5">
          ...
          <p className="mt-1 text-sm text-muted">
            Don’t open these. Real banks and companies won’t rush you through odd links.
          </p>
